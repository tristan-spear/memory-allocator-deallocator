...
BYTE *mymalloc(unsigned long size)
{
    ...
    if (!startofheap)
    {
        startofheap = (BYTE*)sim.sbrk(pages * PAGE_SIZE);
        chunkhead firstChunk = {pages, 1, NULL, NULL};
        memcpy(startofheap, &firstChunk, sizeof(chunkhead));
        return (BYTE *)((BYTE*)startofheap + sizeof(chunkhead));
    }
    else
    {
        ...
        if (current->inuse == 0)
        {
            ...
            {
                chunkhead emptyChunk = {current -> size - pages, 0, current -> next, (BYTE*)current};
        ...
        else
        {
            ...
            current->next = (BYTE *)sim.sbrk(pages * PAGE_SIZE);
...
BYTE *myfree(BYTE *address)
{
    ...
    // if (address < (BYTE *)startofheap || address > (BYTE *)sim.sbrk(0))
    //     return NULL;
    ...
    if (!(freeChunk->next))
    {
        ...
        sim.sbrk(-1 * (int)(freeChunk->size) * PAGE_SIZE);
        return (BYTE *)sim.sbrk(0);
    }
    ...
    else
    {
        ...
        //sim.sbrk(-1 * (freeChunk->size) * PAGE_SIZE);
        return (BYTE *)sim.sbrk(0);
    }
}
...
void analyze()
{
    ...
    if (!startofheap)
    {
        printf("no heap, program break on address: %ld\n", sim.sbrk(0));
        return;
    }
    ...
    {
        printf("%d | current addr: %ld |", no, (BYTE*)ch);
        printf("size: %d | ", ch->size * PAGE_SIZE);
        printf("info: %d | ", ch->inuse);
        printf("next: %ld | ", ch->next);
        printf("prev: %ld", ch->prev);
        printf("      \n");
    }
    printf("program break on address: %ld\n", sim.sbrk(0));
}
